<template>
    <v-main class="list">
        <v-card style="overflow:hidden; box-shadow:0px 2px 6px rgba(0,0,0,0.05); background-color:white">
            <v-card-title>
                <v-text-field v-model="search" append-icon="mdi-magnify" label="Search Merchandise" single-line hide-details>
                </v-text-field>
                <v-spacer></v-spacer>
                        ...
                        <v-card-text>
                            <v-container v-if="inputType == 'Tambah'">
                                ...
                                <v-select filled rounded v-model="selectCategory" :items="itemscategory"
                                    item-text="merchandisectg_name" item-value="merchandisectg_id " label="Choose Category Merchandise"
                                    persistent-hint return-object single-line style="width:400px;"></v-select>
                                <v-text-field filled rounded v-model="form.merchandise_name"
                                    label="Merchandise Name" required>
                                </v-text-field>
        ...
        <v-dialog max-width="300" v-model="dialogPhotos">
            <v-card>
                <v-container>
                   <v-img width="100%" :src="previewImageUrl == '' ? $baseUrl+'/storage/'+form.merchandise_picture : previewImageUrl"
                    id="previewImage" class="mb-5"></v-img>
                </v-container>
            </v-card>
        </v-dialog>
