<template>
    <v-main class="list">
        <v-card style="overflow:hidden; box-shadow:0px 2px 6px rgba(0,0,0,0.05); background-color:white">
            <v-card-title>
                <v-text-field v-model="search" append-icon="mdi-magnify" label="Search Video" single-line hide-details>
                </v-text-field>
                <v-spacer></v-spacer>
                <v-btn style="background-color:#0165BC; color:white" @click="dialog = true"> Add Video </v-btn>
            </v-card-title>
                    ...
                    <v-card>
                        <v-card-text>
                            <v-container v-if="inputType == 'Tambah'">
                                ...
                                <v-text-field filled rounded v-model="form.gallery_videopath"
...
<script>
    export default {
        ...
        methods: {
            ...
            save() {
                this.video.append('gallery_videopath', this.form.gallery_videopath);

                var url = this.$api + '/video'
                this.load = true;

                this.$http.post(url, this.video, {
                    headers: {
            ...
            update() {
                ...
                data.append('gallery_videopath', this.form.gallery_videopath);
                data.append('_method', 'PUT');

                var url = this.$api + '/video/' + this.editId;
                this.load = true;
            ...
            deleteData() {
                var url = this.$api + '/video/' + this.deleteId;
                this.load = true;
            ...
            editHandler(item) {
                this.inputType = 'Ubah';
                this.editId = item.video_id;
                this.form.gallery_videopath = item.gallery_videopath;
                this.dialog = true;
            },
            deleteHandler(item) {
                this.deleteId = item.video_id;
